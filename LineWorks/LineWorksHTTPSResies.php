...
class LineWorksResies{
	...
	function RecvCallBackEvent():bool
	{
		...
		if( false == $ret ){
			//取得失敗
			DEBUG_LOG(basename(__FILE__),__FUNCTION__,__LINE__,"[ERROR]Failed to get user status.user_address=".$tmpUser);
			return $ret;
		}
		...
		if( empty($userStatus->info["user_status"]) ){
			...
			if( false == $ret ){
				//登録失敗
				DEBUG_LOG(basename(__FILE__),__FUNCTION__,__LINE__,"[ERROR]Failed to regist user.user=".$addRegisteredUser->info["user_address"]);
				return $ret;
			}
